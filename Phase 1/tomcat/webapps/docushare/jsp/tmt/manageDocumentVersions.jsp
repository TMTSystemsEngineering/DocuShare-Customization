<%
/**
 * <p>Title: manageDocumentVersions.jsp</p>
...
ResultSet rs = null;
String sql = null;
Vector documentVersions = new Vector();
...
if (btnSubmit != null && "Add".equals(btnSubmit)) {
  success = PicklistManager.addDocumentVersion(request);
  if (success) {
    msgString = "Document Version <b>" + request.getParameter("name") + "</b> added successfully";
  }
  else {
    msgString = (String) request.getAttribute("ADMIN_DOCUMENT_VERSION_ERROR");
  }
}

if (btnSubmit != null && "Delete".equals(btnSubmit)) {
  success = PicklistManager.deleteDocumentVersion(request);
  if (success) {
...
if (btnSubmit != null && "Update".equals(btnSubmit)) {
  success = PicklistManager.updateDocumentVersion(request);
  if (success) {
...
try {
  ...
  sql = "select * from tmt_document_versions order by sequence";
  ...
  while (rs.next()) {
    ...
    documentVersions.addElement(hash);
  }
  rs.close();
  stmt.close();
}
catch (Exception ex) {
  System.out.println("Exception ex in manageDocumentVersions.jsp: " + ex);
...
<%
int tmtCurrentPageHightlightIndex = 9;
String pageTitle = "Document Versions Picklist Management";
%>
...
function validateForm(frm) {
  if (frm.name.value == "") {
    alert("Please enter a three letter abbreviation of the Document Version.");
    frm.name.focus();
    return false;
  }

  if (frm.description.value == "") {
    alert("Please enter a Description for the Document Version.");
...
<%
  ...
  for (int x=0; x < size; x++) {
    Hashtable hash = (Hashtable) documentVersions.elementAt(x);
...
<table cellpadding="3">
<tr><td>Enter the information for a new Document Version to add. </td></tr>
</table>
<table cellpadding="3">
<tr>
  <td align="right"><b>Document Version(3 letter abbreviation):</b></td>
...
<hr/>
<h1>Current Document Versions</h1>
...
%>
  <tr>
    <th>No Document Versions found in database</th>
  </tr>
<%
  }
