<%
/**
 * <p>Title: manageDocumentTypes.jsp</p>
...
if (btnSubmit != null && "Add".equals(btnSubmit)) {
  success = PicklistManager.addDocumentType(request);
  if (success) {
    msgString = "Document Type <b>" + request.getParameter("description") + "</b> added successfully";
  }
  else {
    msgString = (String) request.getAttribute("ADMIN_DOCUMENT_TYPE_ERROR");
  }
}

if (btnSubmit != null && "Delete".equals(btnSubmit)) {
  success = PicklistManager.deleteDocumentType(request);
  if (success) {
    msgString = request.getParameter("description") + " deleted";
  }
  else {
    msgString = (String) request.getAttribute("ADMIN_DOCUMENT_TYPE_ERROR");
  }
...
if (btnSubmit != null && "Update".equals(btnSubmit)) {
  success = PicklistManager.updateDocumentType(request);
  if (success) {
    msgString = request.getParameter("description") + " updated";
  }
  else {
    msgString = (String) request.getAttribute("ADMIN_DOCUMENT_TYPE_ERROR");
  }
...
try {
  ...
  sql = "select * from tmt_document_types order by sequence";
  ...
  while (rs.next()) {
    ...
    documentTypes.addElement(hash);
  }
  rs.close();
  stmt.close();
}
catch (Exception ex) {
  System.out.println("Exception ex in manageDocumentTypes.jsp: " + ex);
...
<%
int tmtCurrentPageHightlightIndex = 7;
String pageTitle = "Document Types Picklist Management";
%>
...
function validateForm(frm) {
  if (frm.name.value == "") {
    alert("Please enter a three letter abbreviation of this Document Type.");
    frm.name.focus();
    return false;
  }

  if (frm.description.value == "") {
    alert("Please enter a Description for this Document Type.");
...
<%
  ...
  for (int x=0; x < size; x++) {
    Hashtable hash = (Hashtable) documentTypes.elementAt(x);
...
<table cellpadding="3">
<tr><td>Enter the information for a new Document Type to add. </td></tr>
</table>
<table cellpadding="3">
<tr>
  <td align="right"><b>Document Type (3 letter abbreviation):</b></td>
...
%>
  <tr>
    <th>No Document Types found in database</th>
  </tr>
<%
  }
